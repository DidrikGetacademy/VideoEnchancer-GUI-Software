...
class KimVocal:
    ...
    def __init__(self,inference_session):
        self.inference_Session_Audio = inference_session
    ...
    def demix_vocals(self, music_tensor, sample_rate, model):
        ...
        Returns:
            torch.Tensor: Output tensor after passing through the network.
        """
        if self.inference_Session_Audio == None:
            raise ValueError("Inference session in kimvocal is None")
        ...
        while i < number_of_samples + pad_size:
            ...
            with torch.no_grad():
                x = torch.tensor(self.inference_Session_Audio.run(None, {"input": x.cpu().numpy()})[0])
...
def main(Model_path,uploaded_file,inference_session):
    INPUT_FOLDER = "./"
    OUTPUT_FOLDER = "./"
    ...
    DidrikVocal = KimVocal(inference_session)
    vocals_tensor = DidrikVocal.demix_vocals(
            music_tensor=music_tensor,
            sample_rate=samplerate,
            model=model_raw_python,
        )
    vocals_array = vocals_tensor.numpy()
    return vocals_array, samplerate
